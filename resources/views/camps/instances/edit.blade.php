<x-layouts.app :title="__('Edit Camp Instance - ' . $instance->name)">
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
        <!-- Header -->
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Edit Camp Instance</h1>
                <p class="text-neutral-600 dark:text-neutral-400">{{ $camp->display_name }} - {{ $instance->year }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div class="p-6">
                <form action="{{ route('camps.instances.update', [$camp, $instance]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="number" id="year" name="year" value="{{ old('year', $instance->year) }}" required min="2020" max="2030"
                            ...
                            <div>
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $instance->name) }}" placeholder="{{ $camp->display_name }} {{ $instance->year }}"
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="3" placeholder="Description for this year's camp session"
                                    class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-zinc-800 text-neutral-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $instance->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="theme_description" name="theme_description" rows="3" placeholder="Theme and special focus for this year"
                                    class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-zinc-800 text-neutral-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('theme_description') border-red-500 @enderror">{{ old('theme_description', $instance->theme_description) }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $instance->start_date?->format('Y-m-d')) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $instance->end_date?->format('Y-m-d')) }}" required
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" id="max_capacity" name="max_capacity" value="{{ old('max_capacity', $instance->max_capacity) }}" min="1"
                                ...
                                <div>
                                    ...
                                    <input type="number" id="price" name="price" value="{{ old('price', $instance->price) }}" min="0" step="0.01"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" id="registration_open_date" name="registration_open_date" value="{{ old('registration_open_date', $instance->registration_open_date?->format('Y-m-d')) }}"
                                ...
                                <div>
                                    ...
                                    <input type="date" id="registration_close_date" name="registration_close_date" value="{{ old('registration_close_date', $instance->registration_close_date?->format('Y-m-d')) }}"
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" id="age_from" name="age_from" value="{{ old('age_from', $instance->age_from) }}" min="0" max="21"
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="age_to" name="age_to" value="{{ old('age_to', $instance->age_to) }}" min="0" max="21"
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select id="grade_from" name="grade_from"
                                            ...
                                            @for($i = 1; $i <= 12; $i++)
                                                <option value="{{ $i }}" {{ old('grade_from', $instance->grade_from) == $i ? 'selected' : '' }}>
                                                    {{ $i }}{{ $i == 1 ? 'st' : ($i == 2 ? 'nd' : ($i == 3 ? 'rd' : 'th')) }} Grade
                                                </option>
                                            @endfor
                                            <option value="13" {{ old('grade_from', $instance->grade_from) == 13 ? 'selected' : '' }}>Graduated Senior</option>
                                        </select>
                                        @error('grade_from')
                                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div>
                                        ...
                                        <select id="grade_to" name="grade_to"
                                            ...
                                            <option value="">Select Grade</option>
                                            @for($i = 1; $i <= 12; $i++)
                                                <option value="{{ $i }}" {{ old('grade_to', $instance->grade_to) == $i ? 'selected' : '' }}>
                                                    {{ $i }}{{ $i == 1 ? 'st' : ($i == 2 ? 'nd' : ($i == 3 ? 'rd' : 'th')) }} Grade
                                                </option>
                                            @endfor
                                            <option value="13" {{ old('grade_to', $instance->grade_to) == 13 ? 'selected' : '' }}>Graduated Senior</option>
                                        </select>
                                        @error('grade_to')
                                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                        @enderror
                    ...
                    <div class="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                        <div class="flex items-center">
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $instance->is_active) ? 'checked' : '' }}
